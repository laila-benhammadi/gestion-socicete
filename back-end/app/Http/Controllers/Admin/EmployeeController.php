...
class EmployeeController extends Controller
{
    ...
    public function addEmployee(Request $request)
    {
        ...
        return response()->json([
            'user' => $user->makeHidden('work_files'),
            'work_files' => json_decode($user->work_files),
        ]);
    }

    public function getEmployeeActif()
    {
        $employees = User::where('is_admin', '!=', 1)
            ->whereHas('experiences', function ($query) {
                $query->whereNull('end_date'); // expérience toujours active
            })
            ->count();
    ...
    {
        $expereince = Experience::with('user')
            ->where('type_contract', 'Stage')
            ->whereNull('end_date')
            ->count();
