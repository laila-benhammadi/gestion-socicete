    ...
    public function getEmployeesOnHoliday()
{
    $users = User::where('is_admin', '!=', 1)
        ->whereHas('experiences', function (EloquentBuilder $query) {
            $query
                // Vérifie que l'expérience a une date de fin ou est encore active
                ->where(function ($query) {
                    $query->whereDate('end_date', '>=', \Carbon\Carbon::now())
                        ->orWhereNull('end_date');
                })
                // Sélectionne la dernière expérience de chaque utilisateur
                ...
                ->whereHas('holidays', function (EloquentBuilder $query) {
                    $query
                        // Vérifie que la date de fin de vacances est NULL (pas encore terminée)
                        ->whereNull('end_date');
                });
        })
        ->with('experiences.holidays')  // Charge les relations expériences et vacances
        ->get();
    return $users;
}
    ...
    {
        $holidayCount = Holiday::with('experience')  
    ...
    {
        $experience = Experience::with('holidays')->findOrFail($id);
